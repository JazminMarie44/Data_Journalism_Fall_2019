...
# Load data and store it as an object called addresses
addresses <- read_csv("data/buyer_addresses.csv")

# Load data and store it as an object called annuel_year
annual_year <- read_csv("data/buyer_annual_by_year.csv")

# Load data and store it as an object called totals
totals <- read_csv("data/buyer_totals.csv")

# Load data and store it as an object called population_year
population_year <- read_csv("data/state_population_per_year.csv")
...
**Task** Answer the following question in a comment in a code block below.  Look at the data.  The four data sets describe similar things -- buyers -- but have different numbers of records? What's your best guess for why the number of records buyer_annual_by_year is higher than buyer_totals?  What about your best guess for why buyer_addresses is higher than buyer_totals? 

```{r}

#Buyer_annual_by_year is higher because there are multiple entriees for the same year whereas the Buyer_totals is grouping by dea number to get the total number of pills. 

#Buyer_addresses is higher than totals because the pharmacy can have multiple addresses which means more entries while the totals simply sorts by DEA number. 
...
#Veterans Affairs Consolidated Mail Outpatient Pharmacy located at 3725 RIVERS AVENUE, North Charleston South Carolina 29405. This pharmacy has so many because it sends pills to patients enrolled in veteran health care across the entire country. 
...
annual_year_addresses <- addresses %>%
  inner_join(annual_year, by="buyer_dea_no") 
```

```{r}
annual_year_addresses %>%
  filter(buyer_state.x == "MD", year == "2010", buyer_bus_act.x == "PRACTITIONER") %>%
  arrange(desc(dosage_unit))


#The name of the practitioner is Rakesh Mathur. In 2012, she was suspended for 5 years and given a fine of 25,000. As of 2015 her suspension was terminated but she is still terminated. On the website, they give you the option to put in the practitioner's license number. 
...
```{r}
  totals %>%
   inner_join(population_year, by="buyer_state") %>%
...
#West Virginia had the highest rate of total pills. 
...
totals_addresses_ <- addresses %>%
  anti_join(totals, by="buyer_dea_no") 
...
# There are 130,000 addresses included in this table that are not in the buyer table. I figured this out by doing an inner join and a left join starting with the addresses and then compariing the number of rows. The fact that it returned more rows than the inner join shows that entries were excluded because they didn't have a match in the totals table. 
```
