...
```{r}
# In the not in population table it is pretty clear that all the entries are pertaining to Puerto Rico. This means that the population table doesn't contain any information on Puerto Rico. In the future this may affect the answers we recieve while working in the population table with numercial data or analysis. The answers may be more or less skewed, or less representative of the country as a whole or the widespread impact of the opioid epidemic.  
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills_per-person',title="North & South Dakota, and Illinoise have highest pills per person", subtitle = "2009 pills per person, U.S. Census", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title=" Pills per person trend from 2006 to 2012", subtitle = "Pills per person starts to gradually decrease between 2010 to 2012, but stay relatives the same in years before", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
