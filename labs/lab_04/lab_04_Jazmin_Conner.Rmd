...
mutate(reporter_address_complete = paste0(reporter_address1," ",reporter_city, ", ", reporter_county, " ",reporter_state, " ", reporter_zip))  
...
mingo_task2 <- mingo %>%
  mutate(out_of_state_distribution = if_else(reporter_state == "WV", "in_state distribution","out_of_state_distribution")
  )
...
#The most significant combo or in other words the combo with the highest amount of pills is Actavis Pharma, Inc, Miami-Luken and Tug Valley Pharmacy. 
...
mingo %>%
  group_by(reporter_name,reporter_dea_no, reporter_city,reporter_addl_co_info) %>%
  summarise(shipments = n()) %>%
  arrange(desc(shipments))
...
#I could use the case_when() function to detect if its "Cardinal Health, Inc." then within in that function do a mutate function to change the name of it to just Cardinal Health. Then perhaps I could do that same process for "Cardinal Health 110, LLC" to change it again just to Cardinal Health. 
