...
ggplot(logan_county_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="In Logan County, opioids maintain 6 million or more through 2012", subtitle = "Total pills per year shipped to Logan County", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

# In Logan County opioid shipments maintained 6 million or more through 2006 to 2012, but this doesn't negate the slight increase of shipments from 2006 to 2009. The year 2009 is when Logan County reaches its peak in opioid shipments, but there is a small spike between 2010-2011 before the number of shipments fall to a little over 6 million agian in 2012. 
...
WESTvirginia_pills_per_year <- arcos_county_pills_per_year %>%
...
ggplot(WESTvirginia_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit, fill=buyer_county)) +
  labs(x="Year", y="Total pills", title="Steady rise in opioids in six large West Virginia counties", subtitle = "Total pills per year shipped to Kanawha, Cabell, Mingo, \nRaleigh, Logan, and Wood counties", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
...
#Kanawha county shows a very steady upwards trend and so does Raleigh, but with a lower number of opioid shipments. Logan and Wood county fairly maintain their the number of opiod shipments. Mingo looks something like a right skewed normal curve and Cabell something like a normal curve. 
...
maryland_pills_per_year <- arcos_county_pills_per_year %>%
  ...
  select(year, dosage_unit, buyer_county, countyfips)

```

```{r}
maryland_rate_2012 <- maryland_population_2012 %>%
  inner_join(maryland_pills_per_year, by=("countyfips")) %>%
  mutate(buyer_county = buyer_county.y) %>%
...
maryland_rate_2012 <- maryland_rate_2012 %>%
  mutate(pills_person_person = (dosage_unit / population))

```


```{r}
maryland_final <- maryland_rate_2012 %>%
...
ggplot(maryland_final) +
  geom_point(aes(estimate, pills_person_person)) +
  labs(x="Income estimate", y="pills per person", title="Maryland Counties pills per person rate and median household income", subtitle = "Weak relation between pills per person dropping as median household income increases", caption = "Source: DEA ARCOS database, via Washington Post")+
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) +
  geom_smooth(aes(estimate, pills_person_person), method = "lm", se = FALSE) +
  geom_text_repel(aes(estimate, pills_person_person, label=buyer_county))
...
#My data project is about opiod shipments being higher during certian seasons or times of the month. Can this potentially be displayed in a scatterplot comparing opioids per month with the state? I'm not sure how to display a graphic of that without downloading raw data that will crash my computer. 
