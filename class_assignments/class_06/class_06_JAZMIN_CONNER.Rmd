...
```{r}
Mont_pharmacies <- not_pharmacies(key = key)

```

```{r}
#Mont_buyer <- buyer_details(county="Montgomery", state="MD", key = key)

```

```{r}

#Mont_pills <- combined_buyer_annual(county="Montgomery", state="MD", key = key)
...
```{r}
#I wanted to investigate whether Montgomery County the place I live was hit hard with the opiod epidemic. One single entry shows that Cardinal Health sent over 43 million pills alone. 

#Mont_manu_pills <- total_distributors_county(county="Montgomery", state="MD", key = key)

```

```{r}
#Montgomery <- total_manufacturers_county(county="Montgomery", state="MD", key = key)

```

```{r}
Montgomery <- county_population(county="Montgomery", state="MD", key = key)

```

```{r}
#I have this theory that opioid pills increase during certain months more than others so I'm trying to find maybe a county that is representative of that. 

# We discussed in class exploring the idea that opioid sales rise more during certain months. Is this a national trend? Does it vary because of different reasons? Are these reasons mainly economic based or holidays?

Montgomery <- summarized_county_monthly(county="Montgomery", state="MD", key = key)

Mingo <- summarized_county_monthly(county="Mingo", state="WV", key = key)

West_Palm_Beach <- summarized_county_monthly(county="West Palm Beach", state="FL", key = key)
...
```{r}
Montgomery <- county_raw(county="Montgomery", state="MD", key = key)
